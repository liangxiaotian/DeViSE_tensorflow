...
class AlexNetSolver(Solver):
    ...
    def __init__(self, dataset, net, common_params, solver_params, net_params):
        ...
        self.fine_tune_projection_layer_dir = str(solver_params['fine_tune_projection_layer_dir'])
    ...
    def construct_graph(self):
        ...
        self.embedding = self.net.inference(self.images, self.keep_prob, self.is_training)
        # loss
        self.total_loss = self.net.rank_loss(self.embedding, self.labels, self.dataset['label_embedding'])
        self.train_acc = self.net.test(self.embedding, self.labels, self.dataset['label_embedding'])
        ...
        with tf.Session(config=tfconfig) as sess:
            ...
            for step in range(self.max_iterators):
                ...
                if step % 1000 == 0 and step != 0:
                    saver2.save(sess, self.train_dir + self.fine_tune_projection_layer_dir + '/model.ckpt',
                                self.global_step)
